...
\newcommand{\jda}[1]{[jda brainstorming: #1]}
...
Here we leverage the language people use to describe a diverse collection of 2K novel objects to --- how object structure is represented in the mind.
...
\noindent The world is filled with a great variety of objects, yet people have no trouble finding ways to communicate about them. 
...
%%%%%%%
The high degree of shape variation in our stimulus set provided us with the opportunity to explore the relationship between the complexity of an object and how many words participants used to describe it.
For these analyses, we operationalize object complexity as the length of the shortest graphics program needed to recreate it, written using simple shape primitives in a ``base'' library (i.e., blocks, lines).
Prior work has suggested a systematic but non-linear relationship between this measure of  object complexity and the length of their linguistic descriptions \cite{sun2021seeing}.
...
To evaluate this hypothesis, we fit a mixed-effects model to predict the number of words entered (into the \textit{what} text boxes) from program length (Fig. \ref{fig:words}A). %the number of program tokens required to recreate the object (Fig. \ref{fig:words}A).
...
Moreover, even the best-fitting quadratic model failed to capture a large proportion of the variation in language using only the length of programs written only in terms of simple shape primitives.
...
While none of the items in our stimulus set look exactly like real-world objects, several of them are nevertheless \textit{evocative} of real-world categories (e.g., vehicles, furniture, houses). 
...
However, examining the relative word frequency distribution produced greater discriminability by subdomain for \textit{technical drawings} ($\Delta F = 12.5$, 95\% CI: [$9.46$, $16.0$], $p<0.001$) than for {block towers} ($\Delta F = 1.47$, 95\% CI: [$0.0441$, $2.83$], $p=0.088$). \textbf{JUDY/WILL TALK ABOUT THIS.}
...
Moreover, we discovered that people often used different words to describe otherwise visually similar objects, although this was more pronounced for the \textit{technical drawings}.
...
\paragraph{People name part concepts corresponding to these moderately complex libraries}
...
In future work, we aim to measure how precisely linguistic descriptions describe the stimuli by showing naive participants the descriptions and asking them to pair parts of the descriptions with parts of the objects.
...
%communicative goal
One form of contextual variation is the speaker's communicative goal.
The requirement for producing instructions, kept constant in the current study, may favor finer-grained, procedural language, in comparison to, say, reference game contexts in which visually salient details might be more important for identification.
